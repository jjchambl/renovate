...
## A Word of Caution
Due to Go's [approach](https://research.swtch.com/vgo-mvs) to module upgrades, upgrading a single module may necessitate an upgrade to another module it lists as a transitive dependency. When this happens, Renovate will make changes to the target dependency as well as any transitive dependencies it lists. This can lead to situations in which the PR summary generated by Renovate does not match the diff against the target branch, particularly when grouping changes.